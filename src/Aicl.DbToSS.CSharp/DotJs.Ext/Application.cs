...
<body>
	<div id=""module""/>	
		...
		public void Write()
		{
			...
			string appDir =  Path.Combine(OutputDirectory, type.Name.ToLower());				
			...
			using (TextWriter tw = new StreamWriter(Path.Combine(appDir, FileName)))
			{
				tw.Write(string.Format(template, AppName, type.Name.ToLower(), type.Name));
				tw.Close();
			}
			
			using (TextWriter hw = new StreamWriter(Path.Combine(appDir, "index.html")))
			{
